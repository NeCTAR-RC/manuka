...
class PendingTestUserApi(base.ApiTestCase):
    ...
    def test_user_get(self):
        response = self.client.get(f"/api/v1/pending-users/{self.user.id}/")

        self.assert200(response)
        self.assertUserEqual(self.user, response.get_json())

    def test_declined_user_get(self):
        response = self.client.get(f"/api/v1/pending-users/{self.user2.id}/")

        self.assert404(response)

    def test_created_user_get(self):
        response = self.client.get(f"/api/v1/pending-users/{self.user3.id}/")

        self.assert404(response)

    def test_user_delete(self):
        response = self.client.delete(f"/api/v1/pending-users/{self.user.id}/")
...
class ProjectsWithRoleTestUserApi(TestUserApiBase):
    ...
    def test_user_projects(self, mock_get_roles, mock_ks_auth, mock_get):
        ...
        response = self.client.get(
            f"/api/v1/users/{self.user.keystone_user_id}/projects/role1/"
        )
        self.assert200(response)
        ...
        mock_client.role_assignments.list.return_value = [mock_as_1, mock_as_2]
