...
class FakeUser:
    def __init__(
        self, username="fakeuser", name="Fake User", email="fake@nectartest"
    ):
        self.full_name = name
...
class TestUtils(base.TestCase):
    ...
    def test_add_user_roles(self, mock_get_roles):
        ...
        calls = [
            mock.call(user=user, project=project, role=role1),
    ...
    def test_get_domain_for_idp(self):
        mapping = models.DomainIdpMapping(
            domain_id="domain123", idp_entity_id="https://idp2"
        )
    ...
    def test_set_swift_quota(self, mock_get_swift):
        ...
        client.post_account.assert_called_once_with(
            headers={
                "x-account-meta-quota-bytes": quota_gb * 1024 * 1024 * 1024
            }
        )
    ...
    def test_refresh_orcid_unknown(self, mock_get_client):
        ...
        mock_client.search_by_email.assert_called_once_with(db_user_email)
